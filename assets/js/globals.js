 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_clearfy_000.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_clearfy_000.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_clearfy_000 ) {
		$.wbcr_factory_clearfy_000 = {};
	}

	$.wbcr_factory_clearfy_000.app = $.wbcr_factory_clearfy_000.app || {
		/**
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wbcr_factory_clearfy_000.hooks.run('wbcr/factory_clearfy_000/hidded_notice', [noticeId]);
				$.wbcr_factory_clearfy_000.hooks.run('wbcr/clearfy/hidded_notice', [noticeId]);
			});
		},
	...
	$.wbcr_factory_clearfy_000.filters = $.wbcr_factory_clearfy_000.filters || {
	...
	$.wbcr_factory_clearfy_000.hooks = $.wbcr_factory_clearfy_000.hooks || {

		/**
		 * Applies filters to a given input value.
		 */
		run: function(filterName, args) {
			$.wbcr_factory_clearfy_000.filters.run(filterName, args);
		},
		 ...
		 */
		add: function(filterName, callback, priority) {
			$.wbcr_factory_clearfy_000.filters.add(filterName, callback, priority);
		}
	};
