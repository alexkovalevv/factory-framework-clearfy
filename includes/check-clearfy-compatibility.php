...
class Wbcr_FactoryClearfy_Compatibility extends Wbcr_Factory_Compatibility {
	...
	public function getNoticeText() {
		...
		if ( self::isClearfyActivate() ) {
			$notice_default_text .= '<b>' . __( 'Clearfy warning', 'wbcr_factory_clearfy_000' ) . ':</b>' . '<br>';
			$phrase              = sprintf( __( 'The %s component', 'wbcr_factory_clearfy_000' ), $this->plugin_title );
		} else {
			$notice_default_text .= '<b>' . $this->plugin_title . ' ' . __( 'warning', 'wbcr_factory_clearfy_000' ) . ':</b>' . '<br>';
			$phrase              = sprintf( __( 'The %s plugin', 'wbcr_factory_clearfy_000' ), $this->plugin_title );
		}

		$notice_default_text .= $phrase . ' ' . __( 'has stopped.', 'wbcr_factory_clearfy_000' );
		$notice_default_text .= __( 'Possible reasons:', 'wbcr_factory_clearfy_000' ) . ' <br>';

		$has_one = false;

		if ( ! $this->isPhpCompatibility() ) {
			$has_one     = true;
			$notice_text .= '- ' . sprintf( __( 'You need to update the PHP version to %s or higher!', 'wbcr_factory_clearfy_000' ), $this->required_php_version ) . '<br>';
		}

		if ( ! $this->isWpCompatibility() ) {
			$has_one     = true;
			$notice_text .= '- ' . sprintf( __( 'You need to update WordPress to %s or higher!', 'wbcr_factory_clearfy_000' ), $this->required_wp_version ) . '<br>';
		}

		if ( $this->required_clearfy_version_compatibility && ! $this->checkClearfyVersionCompatibility() ) {
			$has_one     = true;
			$notice_text .= '- ' . sprintf( __( 'You need to update the Clearfy plugin version to %s or higher!', 'wbcr_factory_clearfy_000' ), $this->required_clearfy_version ) . '<br>';
		}
